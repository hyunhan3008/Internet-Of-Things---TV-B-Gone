...
public:
  ...
  void init() {
    tft.fillRect(WIDTH, HEIGHT, BOXSIZE, BOXSIZE, colours2Boxes[0]);
    tft.drawRect(WIDTH, HEIGHT, BOXSIZE, BOXSIZE, WHITE); // select red box
    currentcolor = RED;
  }
...
void loop(void) {
  ...
  */
  ...
  if(p.x < BOXSIZE+WIDTH  && p.x> BOXSIZE )
    if (p.y < HEIGHT  && p.y> HEIGHT-BOXSIZE)
      for (int i = 0; i < 100; i++) {     // Sony protocol requires three repeats of each command
        ...
        irsend.sendPanasonic(0x4004, 0xD00BCB1);  // look at the examples for the IRremote library to get syntax.
